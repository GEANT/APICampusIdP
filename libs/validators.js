...
const processValidation = function (req, res, next) {
    ...
    return new Promise((resolve, reject) => {
        ...
        validateWebComponent(expanded).then(function () {
            ...
            // aa
            if(idpComponent[0].hasOwnProperty(myVocab+'aa') === true){
                let aa = idpComponent[0]['' + myVocab + 'aa'][0];
                if (typeof aa === undefined) {
                    return reject('empty aa property');
                }
                if (checkForType(aa, myVocab + 'autoGenerated') === true) {
            ...
            if (checkForType(sso, myVocab + 'autoGenerated') === true) {
                ...
                if (checkForType(ssoCerts[0], myVocab + 'autoGenerated') === true) {
                    doCerts = true;
                    delete ssoCerts[0]['@type'];
                }
                else {
                    console.log(JSON.stringify(ssoCerts, null,2));
                    //return reject('ceritificates property is not allowed to be autoGenerate. Use "autoGenerated in sso');
